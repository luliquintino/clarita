...
export default function OnboardingPage() {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden px-4 py-8">
      ...
      <div className="absolute bottom-[-15%] right-[-10%] w-[500px] h-[500px] rounded-full bg-clarita-purple-200/30 blur-3xl animate-float" style={{ animationDelay: "1.5s" }} />

      <div className="w-full max-w-2xl mx-auto relative z-10">
        {/* Logo */}
        <div className="text-center mb-6">
          <Image src="/logo-clarita.png" alt="Clarita" width={80} height={64} className="mx-auto mb-2 drop-shadow-md" priority />
        ...
        >
          ...
          {currentStep?.id === "gynecological" && (
            <GynecologicalStep form={form} update={update} />
          )}

          {currentStep?.id === "history" && (
            <HistoryStep form={form} update={update} />
          )}

          {currentStep?.id === "review" && (
            <ReviewStep form={form} gender={gender} />
          )}
          ...
          {currentStep?.id !== "welcome" && currentStep?.id !== "review" && (
            <p className="text-xs text-gray-400 text-center mt-4">
              Todos os campos são opcionais. Preencha o que quiser agora — você pode atualizar depois.
            </p>
          )}
        </div>
    ...
    <div className="space-y-6">
      <div className="text-center py-2">
        ...
        <p className="text-gray-500 leading-relaxed max-w-md mx-auto">
          Estamos felizes em ter você aqui. Para oferecer o melhor cuidado, vamos
          coletar algumas informações sobre você.
        </p>
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        {cepSuccess && (
          <p className="text-xs text-clarita-green-600 mt-1">Endereço preenchido automaticamente!</p>
        )}
  ...
  return (
    <div className="space-y-4">
      <div className="p-4 bg-clarita-purple-50/60 backdrop-blur-sm rounded-2xl border border-clarita-purple-100/40 text-sm text-clarita-purple-700">
        Estas informações ajudam seus profissionais a oferecer um acompanhamento mais
        completo. Tudo é confidencial.
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="relative">
          <Droplets size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" />
          <select
            className="input-field pl-10"
...
function ReviewStep({
  ...
  const sectionThemes: Record<string, { border: string; bg: string; icon: string }> = {
    "Dados Pessoais": { border: "border-l-clarita-green-400", bg: "bg-clarita-green-50/50", icon: "text-clarita-green-500" },
    "Endereço": { border: "border-l-clarita-blue-400", bg: "bg-clarita-blue-50/50", icon: "text-clarita-blue-400" },
    "Saúde": { border: "border-l-clarita-purple-400", bg: "bg-clarita-purple-50/50", icon: "text-clarita-purple-500" },
    "Saúde Feminina": { border: "border-l-clarita-pink-300", bg: "bg-clarita-pink-50/50", icon: "text-clarita-pink-300" },
    "Histórico & Tratamentos": { border: "border-l-clarita-orange-400", bg: "bg-clarita-orange-50/50", icon: "text-clarita-orange-500" },
  };
  ...
  const sections = [
    ...
    },
    {
      ...
      items: [
        {
          label: "Endereço",
          value: [form.street, form.number, form.complement]
            .filter(Boolean)
            .join(", "),
        },
        {
          label: "Local",
          value: [form.neighborhood, form.city, form.state]
            .filter(Boolean)
            .join(" - "),
        },
  ...
  const hasAnyData = sections.some((s) =>
    s.items.some((i) => i.value?.trim())
  );
    ...
    <div className="space-y-4">
      {!hasAnyData && (
        <div className="p-4 bg-white/40 backdrop-blur-sm rounded-2xl border border-white/30 text-sm text-gray-500 text-center">
          Nenhuma informação preenchida. Você pode completar seu perfil depois no menu de configurações.
        </div>
      )}
      ...
      <div className="flex items-start gap-3 p-4 bg-clarita-green-50/60 backdrop-blur-sm rounded-2xl border border-clarita-green-100/40 mt-2">
        ...
        <p className="text-sm text-clarita-green-700">
          Ao concluir, seus dados serão salvos de forma segura. Você poderá
