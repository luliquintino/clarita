...
export default function DigitalTwinPanel({ twin, patientId }: DigitalTwinPanelProps) {
    ...
    return (
      <div className="space-y-6">
        <div className="card text-center py-12">
          ...
          <h3 className="text-lg font-semibold text-gray-700 mb-2">Gêmeo Digital em Construção</h3>
          <p className="text-sm text-gray-500 max-w-md mx-auto mb-6">
            O gêmeo digital precisa de pelo menos 14 dias de dados emocionais para construir o
            modelo mental do paciente. Continue monitorando para desbloquear previsões, correlações
            e análise de tratamento.
          </p>
          <button
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-800">Gêmeo Digital Mental</h3>
            <p className="text-xs text-gray-400">
              Baseado em {twin.data_points_used} dias de dados &middot; Confiança:{' '}
              {Math.round(twin.confidence_overall * 100)}%
            </p>
          </div>
        </div>
        <button
      ...
      <VariableStateCards currentState={twin.current_state} baseline={twin.baseline} />
...
function VariableStateCards({
  ...
  const vars = Object.entries(currentState).filter(([key]) => key in VAR_LABELS);
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      {vars.map(([key, state]) => {
        ...
        const diff = base ? Math.round((state.current - base.mean) * 10) / 10 : null;
        ...
        return (
          <div key={key} className="card p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              {state.trend === 'stable' && <Minus size={14} className="text-gray-400" />}
              {improving && <TrendingUp size={14} className="text-clarita-green-500" />}
              {worsening && <TrendingDown size={14} className="text-red-500" />}
            </div>

            <div className="flex items-end justify-between">
              <div>
                <span className="text-2xl font-bold text-gray-800">{state.current.toFixed(1)}</span>
                {diff !== null && (
                  <span
            ...
            <div className="text-xs text-gray-400 mt-1">
              7d: {state.avg_7d.toFixed(1)} &middot; 30d: {state.avg_30d.toFixed(1)}
            </div>
          </div>
        );
      })}
...
function CorrelationNetwork({ correlations }: { correlations: DigitalTwinCorrelation[] }) {
  const [hoveredEdge, setHoveredEdge] = useState<number | null>(null);

  if (correlations.length === 0) {
    return (
      <div className="card p-6 text-center">
        <p className="text-sm text-gray-400">Correlações serão detectadas com mais dados</p>
      </div>
    );
  }
    ...
    <div className="card p-4">
      ...
      <svg viewBox="0 0 300 260" className="w-full max-w-md mx-auto">
        ...
        {correlations.map((corr, i) => {
          ...
          const color = corr.direction === 'positive' ? '#22c55e' : '#f59e0b';
        ...
        {variables.map((v) => {
          ...
          return (
            <g key={v}>
              <circle cx={pos.x} cy={pos.y} r={18} fill="white" stroke={color} strokeWidth={2.5} />
              <text
                x={pos.x}
                y={pos.y + 1}
                textAnchor="middle"
      ...
      <div className="h-12 flex items-center justify-center">
        ...
        {hoveredEdge === null && (
          <p className="text-xs text-gray-400">Passe o mouse sobre uma conexão para ver detalhes</p>
        )}
      </div>
    </div>
  );
...
function PredictionTimeline({ predictions }: { predictions: DigitalTwinPrediction[] }) {
  if (predictions.length === 0) {
    return (
      <div className="card p-6 text-center">
        <p className="text-sm text-gray-400">Sem previsões no momento — tendências estáveis</p>
      </div>
    );
  }
          ...
          return (
            ...
            >
                ...
                <div className="flex items-center gap-2">
                  {pred.prediction === 'increase' ? (
                    <ArrowUpRight
                      size={16}
                      className={
                        INVERTED_VARS.has(pred.variable) ? 'text-red-500' : 'text-clarita-green-500'
                      }
                    />
                  ) : (
                    <ArrowDownRight
                      size={16}
                      className={
                        INVERTED_VARS.has(pred.variable) ? 'text-clarita-green-500' : 'text-red-500'
                      }
                    />
                  )}
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                <span className="text-xs text-gray-400">Horizonte: {pred.horizon_days} dias</span>
              </div>
            </div>
          );
        })}
...
function TreatmentResponseChart({ responses }: { responses: TreatmentResponse[] }) {
  const statusConfig = {
    positive_response: {
      icon: CheckCircle,
    ...
    <div className="card p-4">
      ...
      <div className="space-y-4">
        {responses.map((resp, i) => {
          ...
          return (
            <div key={i} className="border border-clarita-beige-200/50 rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <p className="text-sm font-medium text-gray-700">{resp.intervention_name}</p>
                  <p className="text-xs text-gray-400">
                    {resp.intervention_date} &middot; {resp.evaluation_window_days} dias de
                    avaliação
                  </p>
                </div>
...
const GENOME_DIMENSIONS = [
  ...
  },
  {
    ...
    description: 'Grau de reatividade a eventos estressores e velocidade de recuperação',
    icon: 'SE',
    color: '#f59e0b',
  },
  {
    ...
    description: 'Ritmo circadiano de energia, picos e vales ao longo do dia e da semana',
    icon: 'PE',
    color: '#60a5fa',
  },
  {
    ...
    description: 'Impacto da qualidade e duração do sono no funcionamento emocional e cognitivo',
    icon: 'IS',
    color: '#a78bfa',
  },
  {
    ...
    description: 'Perfil de resposta a intervenções farmacológicas e psicoterapêuticas',
    icon: 'RT',
    color: '#14b8a6',
  },
];
  ...
  return (
    <div className="card p-6 relative overflow-hidden">
      <div className="absolute top-4 right-4 flex items-center gap-1.5 px-3 py-1 bg-clarita-green-50 border border-clarita-green-200 rounded-full">
        ...
        <span className="text-xs font-semibold text-clarita-green-700">Em Breve</span>
      </div>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="text-lg font-semibold text-gray-800">Genoma Mental</h3>
          <p className="text-xs text-gray-400">
        ...
        <div className="relative flex items-center justify-center">
          <div className="filter blur-[2px] opacity-60">
            <svg viewBox="0 0 200 200" className="w-full max-w-[220px] mx-auto">
              ...
              />
              {innerPoints.map((p, i) => (
                <circle key={i} cx={p.x} cy={p.y} r={3} fill="#22c55e" fillOpacity={0.8} />
              ))}
              {points.map((p, i) => {
      ...
      <div className="mt-4 px-4 py-3 bg-clarita-beige-50 rounded-xl">
        <p className="text-xs text-gray-500 text-center leading-relaxed">
          O Genoma Mental será um vetor psicológico único por paciente, calculado a partir dos dados
          acumulados pelo Gêmeo Digital. Cada dimensão evolui com o tempo, criando um perfil
          dinâmico que ajuda profissionais a personalizar o tratamento.
        </p>
      </div>
    </div>
  );
    ...
    <div className="card p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-800">Ecossistema CLARITA</h3>
            <p className="text-xs text-gray-400">Funcionalidades em desenvolvimento</p>
          </div>
        </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2.5 mb-2">
                <div className={`w-8 h-8 ${feat.bg} rounded-lg flex items-center justify-center`}>
